...
Puppet::Type.type(:cobblerprofile).provide(:profile) do
  ...
  # sets proxy
  def proxy=(value)
    cobbler('profile', 'edit', '--name=' + @resource[:name], '--proxy=' + value)
    @property_hash[:kickstart]=(value)
  end
  ...
  def create
    ...
    self.proxy = @resource.should(:proxy) unless @resource[:proxy].nil? or self.proxy == @resource.should(:proxy)
